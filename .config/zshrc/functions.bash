...
ipa() {
  paru -Sl | awk '{pkg=$1"/"$2; printf "%s%s\n", pkg, ($4=="" ? "" : " \uf03d4")}' | fzf --prompt "❱ " --style full --reverse --color 16 --multi --preview 'paru -Si $(echo {1} | cut -d/ -f2)' | cut -d " " -f 1 | cut -d "/" -f 2 | xargs -ro paru -S
}
path() {
  echo -e ${PATH//:/\\n}
}
